...
@Registry.register_model('frapmerge_dm')
class FRAPMERGE_DQNAgent(RLAgent):
    '''
    ...
    def __init__(self, world, rank):
        ...
        self.ob_generator = LaneVehicleGenerator(self.world, self.inter_obj,
                                                 ["discretize_position_velocity"], in_only=True, average=None)
    ...
    def relation(self):
        ...
        for i in range(len(self.phase_pairs)):
            zeros = np.zeros(len(self.phase_pairs) - 1, dtype=int)
            cnt = 0
    ...
    def _build_model(self):
        ...
        '''
        model = FRAPMERGE_DM(self.dic_agent_conf, self.num_actions, self.phase_pairs, self.comp_mask)
        return model
    ...
    def get_action(self, ob, phase, test=False):
        ...
        :param test: boolean, decide whether is 2023_1202_17 process
        ...
        if self.phase:
            if self.one_hot:
                phase_t = np.concatenate(
                    [utils.idx2onehot(item[1][1], self.action_space.n, self.dic_phase_expansion) for item in samples])
                phase_tp = np.concatenate(
                    [utils.idx2onehot(item[1][5], self.action_space.n, self.dic_phase_expansion) for item in samples])
            else:
    ...
    def load_model(self, e):
        ...
        self.model = FRAPMERGE_DM(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs,
                          self.comp_mask)
        self.model.load_state_dict(torch.load(model_name))
        self.target_model = FRAPMERGE_DM(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs,
                                 self.comp_mask)
...
class FRAPMERGE_DM(nn.Module):
    ...
    def __init__(self, dic_agent_conf, output_shape, phase_pairs, competition_mask):
        super(FRAPMERGE_DM, self).__init__()
        ...
        self.d1 = nn.Linear(self.demand_shape, 128)
        self.d2 = nn.Linear(128, 32)
        self.d3 = nn.Linear(32, self.d_out)
    ...
    def _forward(self, states):
        ...
        # print("xinghzugna:",states.size())
        num_movements = int((states.size()[1] - 1) / self.demand_shape) if not self.one_hot else int((states.size()[1] - len(self.phase_pairs)*132) / self.demand_shape)
        # num_movements=8
        # print("num_movements:", num_movements)
        batch_size = states.size()[0]
        # print("phase_pairs",self.phase_pairs)
        acts = states[:, :1].to(torch.int64) if not self.one_hot else states[:, :len(self.phase_pairs)*132].to(torch.int64)
        # print('acts',acts)
        states = states[:, 1:] if not self.one_hot else states[:, len(self.phase_pairs)*132:]
        states = states.float()
        # print(states.size())
        # print('states',states)
        ...
        for i in range(num_movements):
            ...
            '''需求就在这里'''
            demand = torch.relu(self.d1(demand))
            demand = torch.relu(self.d2(demand))
            demand = self.d3(demand)
            demand = torch.sigmoid(demand)  # size 4
