...
@Registry.register_model('mydqn')
class MYDQNAgent(RLAgent):
    '''
    ...
    def __init__(self, world, rank):
        ...
        self.ob_generator = LaneVehicleGenerator(self.world,  self.inter, ['detailed_lane_pressure'], in_only=True, average=None)

        self.phase_generator = IntersectionPhaseGenerator(world,  self.inter, ["phase"],
                                                          targets=["cur_phase"], negative=False)
        self.reward_generator = LaneVehicleGenerator(self.world,  self.inter, ["pressure"],
                                                     in_only=True, average='all', negative=True)
        self.action_space = gym.spaces.Discrete(len(self.world.id2intersection[inter_id].phases))

        # if self.phase:
        #     if self.one_hot:
        #         self.ob_length = self.ob_generator.ob_length*66*2 + len(self.world.id2intersection[inter_id].phases)
        #     else:
        #         self.ob_length = self.ob_generator.ob_length*66*2 + 1
        # else:
    ...
    def reset(self):
        ...
        inter_id = self.world.intersection_ids[self.rank]
        inter_obj = self.world.id2intersection[inter_id]
        self.ob_generator = LaneVehicleGenerator(self.world, inter_obj, ['detailed_lane_pressure'], in_only=True, average=None)
        self.phase_generator = IntersectionPhaseGenerator(self.world, inter_obj, ["phase"],
                                                          targets=["cur_phase"], negative=False)
        self.reward_generator = LaneVehicleGenerator(self.world, inter_obj, ["pressure"],
                                                     in_only=True, average='all', negative=True)
    ...
    def _build_model(self):
        ...
        '''
        model = MYDQNNet(self.ob_length, self.action_space.n)
        return model
...
class MYDQNNet(nn.Module):
    '''
    DQNNet consists of 3 dense layers.
    '''
    def __init__(self, input_dim, output_dim):
        super(MYDQNNet, self).__init__()
