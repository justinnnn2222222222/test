...
@Registry.register_model('fraplstm')
class FRAPLSTM_DQNAgent(RLAgent):
    '''
    ...
    def _build_model(self):
        ...
        '''
        model = FRAPLSTM(self.dic_agent_conf, self.num_actions, self.phase_pairs, self.comp_mask)
        return model
    ...
    def load_model(self, e):
        ...
        self.model = FRAPLSTM(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask)
        self.model.load_state_dict(torch.load(model_name))
        self.target_model = FRAPLSTM(self.dic_agent_conf, self.dic_phase_expansion, self.num_actions, self.phase_pairs, self.comp_mask)
...
class FRAPLSTM(nn.Module):
    '''
    FRAP captures the phase competition relation between traffic movements through a modified network structure.
    '''
    def __init__(self, dic_agent_conf, output_shape, phase_pairs, competition_mask):
        super(FRAPLSTM, self).__init__()
        self.oshape = output_shape #存储输出形状
        self.phase_pairs = phase_pairs #存储相位对
        self.comp_mask = competition_mask #存储竞争掩码
        self.demand_shape = dic_agent_conf.param['demand_shape'] # Allows more than just queue to be used 需求形状
        ...
        # 加入LSTM层
        self.lstm = nn.LSTM(input_size=self.lane_embed_units, hidden_size=self.lane_embed_units, num_layers=1, batch_first=True)
    ...
    def _forward(self, states):
        ...
        # LSTM处理时序信息
        lstm_out, _ = self.lstm(phase_demands)
        lstm_out = lstm_out[:, -1, :]  # 取LSTM的最后一个输出

        # 打印调试信息
        print(f"phase_pairs: {self.phase_pairs}")
        ...
        for pair in self.phase_pairs:
             # 检查索引是否在范围内
            if pair[0] < lstm_out.size(0) and pair[1] < lstm_out.size(0):
                pairs.append(lstm_out[pair[0]] + lstm_out[pair[1]])
            else:
                raise IndexError(f"pair {pair} out of bounds for lstm_out with size {lstm_out.size(0)}")
    ...
    def forward(self, states, train=True):
        if train:
            print(f"Input states size: {states.size()}")  # 添加这一行
