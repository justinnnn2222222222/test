...
@Registry.register_model('mymplight')
class MYMPLightAgent(RLAgent):
    '''
    ...
    def __init__(self, world, rank):
        ...
        for inter in self.world.intersections:
            ...
            node_idx = self.world.id2idx[node_id]
            node_obj = self.world.id2intersection[node_id]
            tmp_generator = LaneVehicleGenerator(self.world, node_obj, ["detailed_lane_pressure"],
    ...
    def _build_model(self):
        ...
        agents = MYMPLight_InerAgent(self.model, self.optimizer, self.replay_buffer, self.gamma, self.explorer,
...
class MYMPLight_InerAgent(DQN):
    ...
    def batch_act(self, batch_obs, valid_acts=None, reverse_valid=None, test=False):
        ...
        if valid_acts is None: 
            return super(MYMPLight_InerAgent, self).batch_act(batch_obs)
